...
app.get("/listarDados", async(req,res)=>{
  ...
  try{
    ...
    // não mais um array dentro de array.
    let resultadoConsulta = await connection.execute(`
    SELECT VOOS.CODIGO, TO_CHAR(VOOS.DATA_VOO, 'dd/mm/yyyy') AS DATA_VOO, TRECHOS.NOME, TRECHOS.NOME, TO_CHAR(VOOS.HR_SAIDA, 'HH24:MI:SS') AS HR_SAIDA, TO_CHAR(VOOS.HR_CHEGADA, 'HH24:MI:SS') AS HR_CHEGADA, ORIGEM.NOME AS ORIGEM, DESTINO.NOME AS DESTINO
    FROM VOOS 
