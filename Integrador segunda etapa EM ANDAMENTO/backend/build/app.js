...
app.get("/listarTrechos", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        let resultadoConsulta = yield connection.execute(`SELECT TRECHOS.CODIGO, TRECHOS.NOME, TRECHOS.ORIGEM, TRECHOS.DESTINO, ORIGEM.NOME AS ORIGEM_NOME, DESTINO.NOME AS DESTINO_NOME, AERONAVES.REFERENCIA AS AERO_NOME, AERONAVES.CODIGO AS AERONAVE
    FROM TRECHOS 
...
app.get("/listarBuscaVoos", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // não mais um array dentro de array.
        let resultadoConsulta = yield connection.execute(`
    SELECT VOOS.CODIGO, TO_CHAR(VOOS.DATA_VOO, 'dd/mm/yyyy') AS DATA_VOO, TRECHOS.NOME, TO_CHAR(VOOS.HR_SAIDA, 'HH24:MI') AS HR_SAIDA, TO_CHAR(VOOS.HR_CHEGADA, 'HH24:MI') AS HR_CHEGADA, ORIGEM.NOME AS ORIGEM, DESTINO.NOME AS DESTINO
    FROM VOOS 
    INNER JOIN TRECHOS ON VOOS.TRECHO = TRECHOS.CODIGO 
    INNER JOIN AEROPORTOS ORIGEM ON TRECHOS.ORIGEM = ORIGEM.CODIGO
    INNER JOIN AEROPORTOS DESTINO ON TRECHOS.DESTINO = DESTINO.CODIGO
    WHERE DATA_VOO = '03/11/2023' AND ORIGEM.NOME = 'Aeroporto de Brasília' AND DESTINO.NOME = 'Aeroporto de Manaus'`);
        cr.status = "SUCCESS";
        cr.message = "Dados obtidos";
        // agora sempre vamos converter as linhas do oracle em resultados do nosso TIPO.
        //cr.payload = (rowsToAeronaves(resultadoConsulta.rows));
